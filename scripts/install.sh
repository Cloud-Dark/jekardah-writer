...
for skill in review-rewrite-content storytelling-content hook-gokil headline-variants no-ai-slop tutur-jabodetabek-urban platform-format content-audit; do
  target=$dest/$skill
  [ ! -e "$target" ] && [ ! -L "$target" ] || {
...
cleanup_partial() {
  ...
  for owned_skill in review-rewrite-content storytelling-content hook-gokil headline-variants no-ai-slop tutur-jabodetabek-urban platform-format content-audit; do
